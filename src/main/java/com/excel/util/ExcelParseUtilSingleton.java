 ...
 * Excel导入导出工具类  单例模式
 */
public class ExcelParseUtilSingleton<E> {
    private  E e;
    private static SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    private int etimes = 0;

    private ExcelParseUtilSingleton(E e) {
        this.e = e;
    }

    private ExcelParseUtilSingleton(){
    }
    private static ExcelParseUtilSingleton instance=new ExcelParseUtilSingleton();

    public ExcelParseUtilSingleton getExcelParseUtil(E e){
        return new ExcelParseUtilSingleton(e);
    }

    public static ExcelParseUtilSingleton getInstance(){
        if (instance==null){
            synchronized (ExcelParseUtilSingleton.class){
                if (instance==null){
                    instance=new ExcelParseUtilSingleton();
                }
            }
        }
        return instance;
    ...
    public  <T> Workbook getWorkBook(List<T> list, ExcelDataFormatter edf) throws Exception {
        ...
        int columnIndex = 0;
        Excel excel = null;
        //Map<Field,Integer> mapOrder=getExcelColumn();
        ...
        for (T t : list) {
            ...
            for (Field field : fields) {
                ...
                if(object instanceof Date){
                    ...
                    if(excel.validate()==true){
                        //正则验证 ^[a-zA-Z0-9_]{1,}$  所有包含一个以上的字母、数字或下划线的字符串
                        String pattern="^[a-zA-Z0-9_]{1,}$";
                        if(field.get(t).toString().matches(pattern)){
                            cell.setCellValue(field.get(t).toString());
                        }else {
                            throw new Exception("字符串不匹配");
                        }
                    }
                    cell.setCellValue(field.get(t).toString());
                }
    ...
    public List<E> readFromFile(ExcelDataFormatter edf, File file) throws Exception {
        ...
        // 标题数组，后面用到，根据索引去标题名称，通过标题名称取字段名称用到 textToKey
     ...
     * 写入到EXCEL中的顺序
     */
   /* public Map<Field,Integer> getExcelColumn() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (field.isAnnotationPresent(Excel.class)) {
                if(field.getAnnotation(Excel.class).num()!=0){
                    int order=field.getAnnotation(Excel.class).num();
                    orders.add(order);
                }
            }
        }
